...
{
    let reply = Arc::new(reply); // Arc allows multiple ownership of the reply object
    ...
    T: Display + Send + Sync + 'static,
{
    loop { // continues to accept new connections
        let (mut socket, _) = listener.accept().await.unwrap(); // accept incoming TCP connections
